...
def get_pending_members(count):
    client = BigQueryHook(gcp_conn_id="dpsl_prd_gcp").get_client()
    query = f"""
...
def ghost_member_management():
    ...
    def sync(members):
        ...
        if not new_members.empty:
            upsert_members(json.dumps(new_members.to_dict(orient="records")))
